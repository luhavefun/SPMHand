...
class HO3D(torch.utils.data.Dataset):
    def __init__(self, transform, data_split):
        ...
        'Pinly_4')
        self.seg_root = cfg.vis_seg_dir
        self.seg_whole_root = cfg.whole_seg_dir
    ...
    def __getitem__(self, idx):
        ...
        if self.data_split == 'train':
            ...
            mask = cv2.imread(os.path.join(self.seg_root, seq, 'seg', id[:5] + 'jpg'))
            mask = mask[:, :, 2:3]
            mask = cv2.resize(mask, (640, 480), cv2.INTER_LINEAR)
            mask_whole = cv2.imread(os.path.join(self.seg_whole_root, seq, id[:5] + 'png'), cv2.IMREAD_GRAYSCALE)
        else:
            mask = None
            mask_whole = None
